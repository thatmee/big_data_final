...
object kafkaStreaming {
  ...
  def getStreamAndBatch(sc:StreamingContext) = {
    val streamParams = Map[String, Object] (
    ...
    val batchParams = Map[String, Object] (
      "bootstrap.servers" -> "nyf-2019211193-0001:9092",
      "key.deserializer" -> classOf[StringDeserializer],
      "value.deserializer" -> classOf[StringDeserializer],
    ...
    val stream = KafkaUtils.createDirectStream[String,String](
      sc,
      PreferConsistent,
      Subscribe[String, String](topics, streamParams)
    )
    val batchStream = KafkaUtils.createDirectStream[String,String](
      sc,
      PreferConsistent,
      Subscribe[String, String](topics, batchParams)
    )
    (stream, batchStream)
  }
  ...
  def streamingCore(sc: StreamingContext, task: String): Unit = {
    val (streamStream, batchStream) = kafkaStreaming.getStreamAndBatch(sc)
    var s: InputDStream[ConsumerRecord[String, String]] = streamStream
    if (task == "batch") {
       s = batchStream
    }
  ...
  def main(args:Array[String]) = {
    ...
    streamingCore(streamingSc, "batch")
    streamingSc.start()
